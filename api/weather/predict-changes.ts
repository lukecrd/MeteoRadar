...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!ai) {
      return res.status(200).json(generateRuleBasedReport('Motore Meteorologico Locale Vercel'));
    }
...
- Rateo di variazione umidità: ${humidityChangeRate > 0 ? '+' : ''}${typeof humidityChangeRate === 'number' ? humidityChangeRate.toFixed(1) : '0'}%/h
...
Restituisci un JSON puro valido con questa struttura esatta:
{
  ...
  "confidence": 95
    ...
    try {
      ...
      return res.status(200).json(generateRuleBasedReport('Motore Meteorologico'));
    }
    ...
    console.error('Error during AI prediction on Vercel:', error?.message || error);
    return res.status(200).json(generateRuleBasedReport('Analisi di Backup'));
  }
}
